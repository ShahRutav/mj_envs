...
class HammerEnvVDV0(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        ...
        if RANDOM_OBJECT:
            mujoco_env.MujocoEnv.__init__(self, curr_dir+'/assets/DAPG_hammer_rd_obj.xml', 5)
        else :
    ...
    def reset_model(self):
        ...
        if RANDOM_OBJECT:
            self.sim.model.geom_type[1] = np.random.randint(4) + 2 # between 2 to 6
            self.sim.model.geom_size[1] = (self.np_random.uniform(0.03, 0.04), self.np_random.uniform(0.03, 0.04), self.np_random.uniform(0.03, 0.04))
